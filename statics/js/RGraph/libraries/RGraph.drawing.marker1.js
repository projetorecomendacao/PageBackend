...
this.coordsText=[];path(this.context,['b','lw',prop.linewidth]);if(prop.shadow){RGraph.setShadow(this,prop.shadowColor,prop.shadowOffsetx,prop.shadowOffsety,prop.shadowBlur);}
this.drawMarker();path(this.context,['c','s',prop.colorsStroke,'f',prop.colorsFill]);RGraph.noShadow(this);var textConf=RGraph.getTextConf({object:this,prefix:'text'});this.context.fillStyle=prop.textColor;RGraph.text({object:this,font:textConf.font,size:textConf.size,color:textConf.color,bold:textConf.bold,italic:textConf.italic,x:this.coords[0][0]-1,y:this.coords[0][1]-1,text:this.text,valign:'center',halign:'center',tag:'labels'});RGraph.installEventListeners(this);if(this.firstDraw){this.firstDraw=false;RGraph.fireCustomEvent(this,'onfirstdraw');this.firstDrawFunc();}
RGraph.fireCustomEvent(this,'ondraw');return this;};this.exec=function(func)
{func(this);return this;};this.getObjectByXY=function(e)
{if(this.getShape(e)){return this;}};this.getShape=function(e)
{var mouseXY=RGraph.getMouseXY(e),mouseX=mouseXY[0],mouseY=mouseXY[1];this.context.beginPath();this.drawMarker();if(this.context.isPointInPath(mouseXY[0],mouseXY[1])){return{0:this,1:this.coords[0][0],2:this.coords[0][1],3:this.coords[0][2],4:0,object:this,x:this.coords[0][0],y:this.coords[0][1],radius:this.coords[0][2],index:0,tooltip:prop.tooltips?prop.tooltips[0]:null};}
...
{var r=this.radius;if(prop.align==='left'){var x=this.markerCenterx,y=this.markerCentery;path(this.context,['a',x,y,r,RGraph.HALFPI,RGraph.TWOPI,false]);path(this.context,['qc',x+r,y+r,x+r+r,y+r+r]);path(this.context,['qc',x+r,y+r,x,y+r]);}else if(prop.align==='right'){var x=this.markerCenterx,y=this.markerCentery;path(this.context,['a',x,y,r,RGraph.HALFPI,RGraph.PI,true]);path(this.context,['qc',x-r,y+r,x-r-r,y+r+r]);path(this.context,['qc',x-r,y+r,x,y+r]);}else{var x=this.markerCenterx,y=this.markerCentery;path(this.context,['a',x,y,r,RGraph.HALFPI/2,RGraph.PI-(RGraph.HALFPI/2),true]);path(this.context,['qc',x,y+r+(r/4),x,y+r+r-2]);path(this.context,['qc',x,y+r+(r/4),x+(Math.cos(RGraph.HALFPI/2)*r),y+(Math.sin(RGraph.HALFPI/2)*r)]);}
