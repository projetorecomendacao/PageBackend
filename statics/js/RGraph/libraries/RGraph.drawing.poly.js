...
{var id=conf.id,coords=conf.coords;this.id=id;this.canvas=document.getElementById(this.id);this.context=this.canvas.getContext('2d');this.colorsParsed=false;this.canvas.__object__=this;this.coords=coords;this.coordsText=[];this.original_colors=[];this.firstDraw=true;this.type='drawing.poly';this.isRGraph=true;this.uid=RGraph.createUID();this.canvas.uid=this.canvas.uid?this.canvas.uid:RGraph.createUID();this.properties={linewidth:1,colorsStroke:'black',colorsFill:'red',eventsClick:null,eventsMousemove:null,tooltips:null,tooltipsOverride:null,tooltipsEffect:'fade',tooltipsCssClass:'RGraph_tooltip',tooltipsEvent:'onclick',tooltipsHighlight:true,highlightStroke:'rgba(0,0,0,0)',highlightFill:'rgba(255,255,255,0.7)',shadow:false,shadowColor:'rgba(0,0,0,0.2)',shadowOffsetx:3,shadowOffsety:3,shadowBlur:5,clearto:'rgba(0,0,0,0)'}
...
{var coords=this.coords;path(this.context,['b','m',coords[0][0],coords[0][1]]);for(var i=1,len=coords.length;i<len;++i){this.context.lineTo(coords[i][0],coords[i][1]);}
path(this.context,['lw',prop.linewidth,'c','f',this.context.fillStyle,'s',this.context.strokeStyle]);};this.getShape=function(e)
{var coords=this.coords,mouseXY=RGraph.getMouseXY(e),mouseX=mouseXY[0],mouseY=mouseXY[1];var old_strokestyle=this.context.strokeStyle,old_fillstyle=this.context.fillStyle;this.context.beginPath();this.context.strokeStyle='rgba(0,0,0,0)';this.context.fillStyle='rgba(0,0,0,0)';this.drawPoly();this.context.strokeStyle=old_strokestyle;this.context.fillStyle=old_fillstyle;if(this.context.isPointInPath(mouseX,mouseY)){return{0:this,1:this.coords,2:0,'object':this,'coords':this.coords,'index':0,'tooltip':prop.tooltips?prop.tooltips[0]:null};}
return null;};this.highlight=function(shape)
{this.context.fillStyle=prop.colorsFill;path(this.context,'c f % s %',prop.highlightFill,prop.highlightStroke);if(prop.tooltipsHighlight){if(typeof prop.highlightStyle==='function'){(prop.highlightStyle)(shape);}else{path(this.context,['b','fu',function(obj){obj.drawPoly();},'f',prop.highlightFill,'s',prop.highlightStroke]);}}};this.parseColors=function()
