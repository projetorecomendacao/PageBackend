...
{var id=conf.id,canvas=document.getElementById(id),x=conf.x,y=conf.y,radius=conf.radius;this.id=id;this.canvas=document.getElementById(this.id);this.context=this.canvas.getContext('2d');this.canvas.__object__=this;this.original_colors=[];this.firstDraw=true;this.centerx=x;this.centery=y;this.radius=radius;this.type='drawing.circle';this.isRGraph=true;this.uid=RGraph.createUID();this.canvas.uid=this.canvas.uid?this.canvas.uid:RGraph.createUID();this.properties={colorsStroke:'rgba(0,0,0,0)',colorsFill:'red',eventsClick:null,eventsMousemove:null,shadow:false,shadowColor:'gray',shadowOffsetx:3,shadowOffsety:3,shadowBlur:5,highlightStyle:null,highlightStroke:'black',highlightFill:'rgba(255,255,255,0.7)',tooltips:null,tooltipsHighlight:true,tooltipsEvent:'onclick',linewidth:1,clearto:'rgba(0,0,0,0)'}
...
path(this.context,'b lw %',prop.linewidth);if(prop.shadow){RGraph.setShadow({object:this,prefix:'shadow'});}
path(this.context,'b a % % % % % % f % s %',this.coords[0][0],this.coords[0][1],this.radius,0,RGraph.TWOPI,false,prop.colorsFill,prop.colorsStroke);RGraph.noShadow(this);RGraph.installEventListeners(this);if(this.firstDraw){this.firstDraw=false;RGraph.fireCustomEvent(this,'onfirstdraw');this.firstDrawFunc();}
RGraph.fireCustomEvent(this,'ondraw');return this;};this.exec=function(func)
{func(this);return this;};this.getObjectByXY=function(e)
{if(this.getShape(e)){return this;}};this.getShape=function(e)
{var mouseXY=RGraph.getMouseXY(e),mouseX=mouseXY[0],mouseY=mouseXY[1];if(RGraph.getHypLength(this.centerx,this.centery,mouseXY[0],mouseXY[1])<=this.radius){return{0:this,1:this.centerx,2:this.centery,3:this.radius,4:null,5:0,'object':this,'x':this.centerx,'y':this.centery,'radius':this.radius,'index':0,'tooltip':prop.tooltips?prop.tooltips[0]:null};}
return null;};this.highlight=function(shape)
{if(prop.tooltipsHighlight){if(typeof prop.highlightStyle==='function'){(prop.highlightStyle)(shape);}else{path({object:this,path:'b a % % % % % % f % s %',args:[this.centerx,this.centery,this.radius+0.5,0,RGraph.TWOPI,false,prop.highlightFill,prop.highlightStroke]});}}};this.parseColors=function()
