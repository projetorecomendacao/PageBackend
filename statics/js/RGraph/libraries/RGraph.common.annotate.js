...
RGraph.Registry.set('annotatable.actions',RGraph.Registry.get('annotatable.actions')+'|'+mouseX+','+mouseY);RGraph.Registry.set('annotatable.last.coordinates',[mouseX,mouseY]);RGraph.fireCustomEvent(obj,'onannotate');obj.context.stroke();}};RGraph.showPalette=RGraph.showpalette=function(e)
{var isSafari=navigator.userAgent.indexOf('Safari')?true:false;e=RGraph.fixEventObject(e);var canvas=e.target.parentNode.__canvas__,context=canvas.getContext('2d'),obj=canvas.__object__,div=document.createElement('DIV'),coords=RGraph.getMouseXY(e)
...
{RGraph.hidePalette();}
e.stopPropagation();return false;};RGraph.clearAnnotations=RGraph.ClearAnnotations=function(canvas)
{if(typeof canvas==='string'){var id=canvas;canvas=doc.getElementById(id);}else{var id=canvas.id}
var obj=canvas.__object__;if(win.localStorage&&win.localStorage['__rgraph_annotations_'+id+'__']&&win.localStorage['__rgraph_annotations_'+id+'__'].length){win.localStorage['__rgraph_annotations_'+id+'__']=[];RGraph.fireCustomEvent(obj,'onannotateclear');}};RGraph.replayAnnotations=RGraph.ReplayAnnotations=function(obj)
