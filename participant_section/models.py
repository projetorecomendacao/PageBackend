...
#    address = models.ForeignKey(Address, on_delete=models.DO_NOTHING, related_name='participant_situations')
    ...
    p12_professional_activite = models.CharField('Atividade Remunerada exercida:',max_length=30, null=True, blank=True)
    p13_income_I = models.CharField('Rendimento Mensal Individual:',max_length=30,choices=Options.INCOME)
    p13_income_F = models.CharField('Rendimento Mensal Familiar:',max_length=30,choices=Options.INCOME)
    p14_lives_with = models.TextField("Com quem mora", null=True, blank=True)
    p15_have_religion = models.CharField("Possui Religião",max_length=1,choices=Options.CHOICES)
